...
is being judged by the reviewer with no automated tool that assist with this
task.
...
The new flag to check for ABI compatibility follows the same rational as the
...
This index file changes have been implemented in version 0.7 of the Python package *rosdistro*.
...
package ``rosdistro`` anymore but are still used by many users this breakage
should be avoided.
