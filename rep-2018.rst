...
There should be optinal arguments to monotor the specific interface types.
...
This can record the action communcation in the bag file aligned with topics and services.
...
It would be really helpful to have GUI base tool to see those data at runtime or from bag file, so that users can determine communication among services and actions with a breif look.
