...
" * " means that this is not the upstream version but a package distributed by OSRF or the community

Maintainers can chose to support even lower versions to also cover older ROS distributions and thus avoid branching out
...
Gazebo
------
As of ROS Lunar, we are using the gazebo packages released by OSRF rather than the upstream version.
This is due to a discrepancy between upstream versions on targeted Ubuntu distributions
