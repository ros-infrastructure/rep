#!/usr/bin/env python
"""Perform an integrity check upon all REPs to make sure the needed svn
properties are set."""
...
def get_props():
    """Return the properties set on rep-*.txt files as an ElementTree instance.

    Files with no properties set will not be contained in the returned data.

    """
    cmd = 'svn proplist --xml rep-*.txt'
...
def missing_props(props):
    """Figure out what properties are missing on what REPs, returning a sequence
    of (path, [missing_props]) pairs.

    For the set properties (as calculated by get_props()), see which REPs are
    lacking any properties. For the REPs that are not even listed in the set
    properties, assume they are missing all needed properties.

    """
    problems = []
    missing_files = set(glob.glob('rep-*.txt'))
    missing_files.remove('rep-0000.txt')
    for target in props:
