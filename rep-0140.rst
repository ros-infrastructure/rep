...
of the package and where always configured (even if not being built
...
separate ``<build_depend>`` is required in that case.  That assumed that
...
all on the same package.  It is not allowed to use the ``<depend>``
tag in combination with any of the three equivalent tags for the same
package name.
...
future similar to how theunit tests are integrated into the configure
...
Generated Debian packages are built without the documentation or their
dependencies.
