...
In addition, if it turns out to be necessary, the simpler approach can always be replaced by a better method for getting the optimal solution selected message type in future work.
...
As with REP 2007, the C++'s templating system will make it much easier handle the various message types expected safely.
