...
:Negotiating Publisher and Subscriber Pair:
  A single node that has pair of negotiating publishers and subscribers that are related to each other in that the subscribers preferences may be based on the selected message type of the publisher.
...
Thus it was an easy decision to extend the lifecycle nodes to work with the negotiation process.
However, the type negotiation feature described in this REP will be in a separate package from the lifecycle nodes to avoid changing the lifecycle nodes.
