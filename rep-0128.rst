...
=====================================
catkin was designed to allow rosbuild packages to sit on top of catkin ones. This is accomplished by using overlays and sourcing the appropriate setup.* files in the correct order, whether they are generated by rosbuild or catkin. Though one could technically utilize the same folder for a rosbuild workspace and a catkin workspace, our recommended layout is to isolate rosbuild and catkin workspaces in their own folders. 
...
If we wanted to build packages in "rosbuild_ws" against those in "catkin_ws", we would simply source the setup file in catkin_ws before calling rosmake on any of the rosbuild packages in rosbuild_ws:
