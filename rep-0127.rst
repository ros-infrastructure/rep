...
Catkin-only specification
-------------------------

The ``package.xml`` file only contains information needed for a ROS
catkin system.  Any additional information necessary for backward
compatibility with rosbuild is kept separate, in the ``manifest.xml``
files.  This makes it easy to remove the legacy files in the future.
Legacy files required for rosbuild interaction are fully generated
from the new ``package.xml`` and the ``CMakeLists.txt``, and need not
...
Metapackages all packages or other metapackages in their group using
``<run_depend>`` tags::
...
For backward compatibility, catkin generates a ``manifest.xml`` which
exports the compiler and linker flags needed by rosbuild.
