...
Declares a ROS dep key for this package that is *both* a
`\<build_depend\>` and a `\<run_depend\>`.
...
Declares a ROS dep key that this package depends on either at run-time
or in an exported build interface.
