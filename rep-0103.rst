...
Note that by the right hand rule, the yaw component of orientation increases as
the child frame rotates counter-clockwise, and for geographic poses, the zero yaw
value is when pointed due east.

This requires special mention only because differs from a traditional compass bearing,
which is zero when due north and incrementing clockwise. Hardware drivers should make
