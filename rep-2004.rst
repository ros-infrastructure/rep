...
The categories address policies about things including version, change control, documentation, testing, dependencies, platform support and others.
...
This REP does not aim to dictate how maintainers meet each of the expectations for each quality level, instead they are allowed to come up with their own policies or follow an existing set of guidelines for achieving these quality levels, for example by using a reference implementation like the one in the "Reference Implementation" subsection below.
For this reason, some of the requirements for each quality level are intentionally generic, avoiding specific recommendations like which tools to use or what specific thresholds should be.

Additionally, this REP does not aim to enforce adherence to the polices documented by package maintainers.
Instead it recommends the creation of peer reviewed lists of packages, where maintainers can take their quality declaration (see below) and have their peers verify that it matches what is stated.
Also, it avoids trying to enforce these things because it cannot be done in an automated fashion in all cases, and requirements that are skipped or altered with justifications (again, see below) require evaluation by the consumer to see if that is acceptable.

The outcome of this REP should be that maintainers have looked at each requirement, thought about how they address each, adjust their policies if needed, and then document their policies and justifications.
