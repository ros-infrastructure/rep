...
* apt_repo: url of the associated apt repository
* sync: specify the criteria that need to be verified for packages to be synced with the
  apt repo. One or both of the following options can be set:

  * minimum: integer. Specifies how many packages need to be successfully built to
    initiate sync. (default: 1)
  * packages: list of packages. All the packages of this list must be successfully built to
...
::
  ...
  apt_repo: http://archives.example.com/ros
  sync:
...
::
  ...
  apt_repo: http://repo.example.com/
  sync:
    minimum: 123
