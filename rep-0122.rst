...
Last-Modified: $Date: 2012-02-20 12:02:31 -0800 (Mon, 20 Feb 2012) $
...
All package-relative executables, scripts and libraries are installed
in ``share/<ros-package-name>/`` subdirectory.
...
stacks following the new REP 122 layout.  Converted packages do not
use those files.
...
In ROS Fuerte, ``ROS_PACKAGE_PATH`` was defined as:

    ROS_PACKAGE_PATH=/opt/ros/fuerte/share:/opt/ros/<distro>/stacks

``ROS_ROOT`` was defined to be::
...
Stack version may could be in stack.yaml
----------------------------------------

Fuerte Catkin defined a ``stack.yaml`` file for release packaging that
contained a version number.  In order to not violate DRY (Don't Repeat
Yourself) principles, tools like ``rosversion`` were capable of
locating the version number defined in this file.  The version number
in the ``stack.yaml`` had a higher precedence.
